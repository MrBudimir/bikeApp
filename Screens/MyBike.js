...
class MyBike extends Component {
  ...
  getCurrentInvoice(emailOfUser) {
    ...
    axios
      ...
      .catch((err) => {
        ...
        if (
          this.isEmpty(this.state.currentInvoice) &&
          !this.isEmpty(this.invoiceFromStorage)
        ) {
          console.log(
            "Could not get invoice from db, instead using invoice from storage if existing",
            err
          );
  ...
  endRent() {
    ...
    const params = {
      params: {
        invoiceId: this.invoiceFromStorage.id
          ? this.invoiceFromStorage.id
          : this.state.currentInvoice.id,
        endDate: this.invoiceFromStorage.endDate
          ? this.invoiceFromStorage.endDate
          : new Date().getTime(),
      },
    };
    ...
    axios
      ...
      .then((response) => {
        this.storage
          .storeData(CURRENT_INVOICE_KEY, {})
          .then((r) => console.log("Stored data of current invoice"));

        this.message.successMessage(
          "End Rent",
          "You successfully ended your rent!"
        );
      })
      .catch((err) => {
        ...
        if (!this.invoiceFromStorage.requestNeeded) {
          console.log(
            "Could not end rent but request will be saved to end rent later",
            err
          );
          ...
          this.storage
            .storeData(CURRENT_INVOICE_KEY, invoiceWithRequestState)
            .then((r) => console.log("Stored data of current invoice"));
        }
      });
    this.setState({
    ...
    if (this.state.startDate) {
      ...
      endButtonOrRefresh = (
        <TouchableOpacity
          ...
          style={
            this.state.startDate ? styles.endRentButton : styles.disabledButton
          }
      ...
      <View style={styles.screen}>
        <View style={styles.invoiceMetaDataView}>
          ...
          <InfoField
            header="Start Date"
            text={
              this.state.currentInvoice.startDate
                ? this.state.currentInvoice.startDate
                : "-"
            }
          />
